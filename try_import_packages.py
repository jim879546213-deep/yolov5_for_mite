...
def return_model(obj_path = "pth\\best_adj.pt", cls_res_path = None,cls_next_path=None,vgg_11_path = None):
    '''
    ...
    if cls_res_path is not None:
        ...
        model_cls.eval()
    elif vgg_11_path is not None:
        model_cls = models.vgg11_bn(pretrained=False)
        ...
        model_cls.to(device_cls)
        model_cls.load_state_dict(torch.load(vgg_11_path))
        model_cls.eval()
...
def cls(y_pred,threshold=0.5):
  y_pred = torch.where(y_pred>threshold, torch.tensor(1), torch.tensor(0))
  return y_pred
